...
func client() {
	...
	// Make data reciver
	dataRecvier := make([]byte, 1024)
	
	// Retrive data and the length of data
	n, err := dial.Read(dataRecvier)
	if err != nil {
        log.Fatal(err)
    }
	...
	fmt.Println(string(dataRecvier[:n]))
	
	// Add 1 to the length of the data recived
	dataAck := make([]byte, 1024)
